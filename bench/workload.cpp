...
int main(int argc, char *argv[])
{
	...
	{
		std::cout << "Usage:\n";
		std::cout << "1. filter type: SuRF, SuRFHash, SuRFReal, SuRFMixed, Bloom, Rosetta, ElasticRosetta\n";
	...
	// check args ====================================================
	if (filter_type.compare(std::string("SuRF")) != 0 && filter_type.compare(std::string("SuRFHash")) != 0 && filter_type.compare(std::string("SuRFReal")) != 0 && filter_type.compare(std::string("SuRFMixed")) != 0 && filter_type.compare(std::string("Bloom")) != 0 && filter_type.compare(std::string("ARF")) != 0 && filter_type.compare(std::string("Rosetta")) != 0 && filter_type.compare(std::string("ElasticRosetta")) != 0 && filter_type.compare(std::string("ElasticBF")) != 0)
	{
	...
	if (workload_type.compare(std::string("mixed")) != 0 && workload_type.compare(std::string("alterByte")) == 0)
	{
	...
	if (key_type.compare(std::string("randint")) != 0 && key_type.compare(std::string("timestamp")) != 0 && key_type.compare(std::string("email")) != 0)
	{
	...
	if (query_type.compare(std::string("point")) != 0 && query_type.compare(std::string("range")) != 0 && query_type.compare(std::string("mix")) != 0 && query_type.compare(std::string("count-long")) != 0 && query_type.compare(std::string("count-short")) != 0)
	{
	...
	if (key_type.compare(std::string("email")) == 0)
		bench::loadKeysFromFile(load_file, false, load_keys, level);
	else
		bench::loadKeysFromFile(load_file, true, load_keys, level);
	std::cout << "size:" << load_keys.size() << std::endl;
	...
	if (key_type.compare(std::string("email")) == 0)
		bench::loadKeysFromFile(txn_file, false, txn_keys, level);
	else
		bench::loadKeysFromFile(txn_file, true, txn_keys, level);
	...
	if (filter_type.compare(std::string("Rosetta")) == 0)
		filter = bench::FilterFactory::createFilter(insert_keys, insert_keys.size(), level, 10);
	...
	else if (filter_type.compare(std::string("ElasticBF")) == 0)
		filter = bench::FilterFactory::createFilter(insert_keys, {5, 5, 5, 5});
	else
		filter = bench::FilterFactory::createFilter(filter_type, suffix_len, insert_keys);
